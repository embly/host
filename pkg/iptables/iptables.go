...
func (e *Error) Error() string {
	return fmt.Sprintf("running %v: exit status %v: %v", e.args, e.ExitStatus(), e.msg)
}
...
func New() (*IPTables, error) {
	return NewWithProtocol(ProtocolIPv4, exec.New())
}

// New creates a new IPTables for the given proto.
// The proto will determine which command is used, either "iptables" or "ip6tables".
func NewWithProtocol(proto Protocol, execInterface exec.Interface) (*IPTables, error) {
	path, err := execInterface.LookPath(getIptablesCommand(proto))
	if err != nil {
		return nil, err
	}
	vstring, err := getIptablesVersionString(path, execInterface)
	if err != nil {
...
func getIptablesVersionString(path string, execInterface exec.Interface) (string, error) {
	cmd := execInterface.Command(path, "--version")
