...
func TestProto(t *testing.T) {
	...
	ip4t, err := NewWithProtocol(ProtocolIPv4, exec.New())
	if err != nil {
	...
	ip6t, err := NewWithProtocol(ProtocolIPv6, exec.New())
	if err != nil {
...
func TestIsNotExist(t *testing.T) {
	...
	defer func() {
		_ = ipt.ClearChain("filter", chainName)
		_ = ipt.DeleteChain("filter", chainName)
	}()
...
func TestIsNotExistForIPv6(t *testing.T) {
	ipt, err := NewWithProtocol(ProtocolIPv6, exec.New())
	if err != nil {
		t.Fatalf("failed to init: %v", err)
	}
	// Create a chain, add a rule
